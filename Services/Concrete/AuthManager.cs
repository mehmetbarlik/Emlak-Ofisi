...
namespace Services.Concrete
{
    public class AuthManager : IAuthService
    {
        ...
        public async Task<IdentityResult> CreateUser(UserDtoForCreation usertDto)
        {
            var user = _mapper.Map<IdentityUser>(usertDto);
            var result = await _userManager.CreateAsync(user, usertDto.Password);

            if (!result.Succeeded)
            {
                throw new Exception("Kullanıcı oluşturulamadı.");
            }
            if (usertDto.Roles.Count > 0)
            {
                var roleResult = await _userManager.AddToRolesAsync(user, usertDto.Roles);
                if (!roleResult.Succeeded)
                {
                    throw new Exception("Rol oluşturualamadı.");
                }
            }
            return result;
        }
